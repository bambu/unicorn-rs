...
#[repr(C)]
pub struct Mode : u32 {
        const LITTLE_ENDIAN = 0; // little-endian mode (default mode)
        ...
        const THUMB = 1 << 4;   // ARM's Thumb mode, including Thumb-2
        const MCLASS = 1 << 5;  // ARM's Cortex-M series
        const V8 = 1 << 6;      // ARMv8 A32 encodings for ARM
        ...
        const BIG_ENDIAN = 1 << 30; /* big-endian mode
